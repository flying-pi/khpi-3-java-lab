...
public class ProductCollection<T extends IBaseProduct> extends AbstractCollection<T>  implements Cloneable{
    private T[] products;
    private int size;
    ...
    @Override
    public ProductCollection<T> clone() throws CloneNotSupportedException {
        ProductCollection<T> result = (ProductCollection<T>) super.clone();
        result.products = (T[]) new IBaseProduct[(int) (products.length)];
        for(int i=0;i<size;i++){
            result.products[i] = (T) products[i].clone();
        }
        result.size = size;
        return result;
    }
